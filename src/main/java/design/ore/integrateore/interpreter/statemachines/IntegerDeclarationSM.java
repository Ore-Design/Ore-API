...
@NoArgsConstructor
public class IntegerDeclarationSM implements IStateMachine
{
	...
	public void validate(Set<String> definedVariables, Iterator<Token> itr)
	{
		...
		while(itr.hasNext())
		{
			...
			if(validStatesCount == 0 && nextType != TokenType.INTEGER_DECLARATION)
			{
			...
			{
				fail("Incomplete syntax, expected spec reference, variable, or literal literal, instead got duplicate operator.");
				...
				else if(nextType == TokenType.CLOSE_PARENTHESIS)
				{
					if(!expectingOperator)
					{
						fail("Incomplete syntax, expected spec reference, variable, or integer literal, instead got parenthesis.");
						unclosedParenthesisCount = 0;
						break;
					}
					else
				...
				else if(nextType == TokenType.INTEGER_LITERAL || nextType == TokenType.VARIABLE || nextType == TokenType.SPEC)
				{
					expectingOperator = true;
					succeed();
				}
				else if(nextType == TokenType.DECIMAL_LITERAL)
				{
					fail("Incomplete syntax, expected spec reference, variable, or integer literal, instead got decimal literal.");
					unclosedParenthesisCount = 0;
					break;
				}
				else if(nextType == TokenType.TEXT_LITERAL)
				{
					fail("Incomplete syntax, expected spec reference, variable, or literal literal, instead got text literal.");
					unclosedParenthesisCount = 0;
					break;
				}
				else if(nextType == TokenType.TRUEFALSE_LITERAL)
				{
					fail("Incomplete syntax, expected spec reference, variable, or literal literal, instead got truefalse literal.");
					unclosedParenthesisCount = 0;
					break;
				}
